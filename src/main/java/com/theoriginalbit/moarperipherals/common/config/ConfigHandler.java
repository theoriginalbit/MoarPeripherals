...
public final class ConfigHandler {
    ...
    public static int blockIdKeyboardMac = startBlockID++;
    @SuppressWarnings("unused")
    private static int blockIdConsume = startBlockID++; // this was the item dictionary, so as to not break things, it still sits consumed
    ...
    public static int blockIdComputerCrafter = startBlockID++;
    public static int blockIdFireworks = startBlockID++;
    public static int blockIdFireworksCreative = startBlockID++;
    public static int blockIdKeyboardPc = startBlockID++;
    public static int blockIdMiniAntenna = startBlockID++;
    public static int blockIdTurtleTeleport = startBlockID++;
    ...
    public static int itemIdSolarPanel = startItemID++;
    public static int itemIdDensityScanner = startItemID++;
    ...
    private void doConfiguration() {
        ...
        // Block ID
        blockIdChatBox = getBlockId(CATEGORY_CHAT_BOX, blockIdChatBox);
        blockIdChatBoxAdmin = getBlockId("ChatBox (Admin)", blockIdChatBoxAdmin);
        blockIdPrinter = getBlockId(CATEGORY_PRINTER, blockIdPrinter);
        blockIdKeyboardMac = getBlockId(CATEGORY_KEYBOARD + " (Mac)", blockIdKeyboardMac);
        blockIdIronNote = getBlockId(CATEGORY_IRON_NOTE, blockIdIronNote);
        blockIdPlayerDetector = getBlockId(CATEGORY_PLAYER_DETECTOR, blockIdPlayerDetector);
        blockIdAntenna = getBlockId(CATEGORY_ANTENNA + " Pole", blockIdAntenna);
        blockIdAntennaCell = getBlockId(CATEGORY_ANTENNA + " Cell", blockIdAntennaCell);
        blockIdAntennaMiniCell = getBlockId(CATEGORY_ANTENNA + " MiniCell", blockIdAntennaMiniCell);
        blockIdAntennaController = getBlockId(CATEGORY_ANTENNA + " Controller", blockIdAntennaController);
        blockIdComputerCrafter = getBlockId("Computer Crafter", blockIdComputerCrafter);
        blockIdFireworks = getBlockId(CATEGORY_FIREWORK, blockIdFireworks);
        blockIdFireworksCreative = getBlockId(CATEGORY_FIREWORK, blockIdFireworksCreative);
        blockIdKeyboardPc = getBlockId(CATEGORY_KEYBOARD + " (PC)", blockIdKeyboardPc);
        blockIdMiniAntenna = getBlockId(CATEGORY_MINI_ANTENNA, blockIdMiniAntenna);
        blockIdTurtleTeleport = getBlockId(CATEGORY_TURTLE_TELEPORT, blockIdTurtleTeleport);

        // Item ID
        itemIdSonic = getItemId(CATEGORY_SONIC, itemIdSonic);
        itemIdInkCartridge = getItemId(CATEGORY_PRINTER + " Ink", itemIdInkCartridge);
        itemIdKeyboardPart = getItemId(CATEGORY_KEYBOARD + " Part", itemIdKeyboardPart);
        itemIdMonopoleAntenna = getItemId("Monopole Antenna", itemIdMonopoleAntenna);
        ...
        config.get(CATEGORY_UPGRADE_FURNACE, "fuelConsumption", upgradeFurnaceFuelConsumption);
        ...
        userDensityMappings = config.get("customDensity", CATEGORY_UPGRADE_DENSITY, userDensityMappings,
...
//        securityOpBreak = getBoolean(CATEGORY_SECURITY, "canOpBreakSecurity", "Are OPs able to break blocks that they don't own (when applicable); It is suggested you have this set to false until needed e.g. griefing");
    ...
    private static double getDouble(String cat, String key, double defDbl, String desc) {
        return config.get(cat, key, defDbl, desc).getDouble(defDbl);
    }
