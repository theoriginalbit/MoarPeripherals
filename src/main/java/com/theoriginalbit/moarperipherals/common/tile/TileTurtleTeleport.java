...
import com.theoriginalbit.moarperipherals.common.network.packet.PacketFxTeleport;
...
public class TileTurtleTeleport extends TileMoarP {
    ...
    private void doTeleportFX(ChunkCoordinates target) {
        if (!worldObj.isRemote) {
            ...
            final PacketFxTeleport sourceMessage = new PacketFxTeleport(dimId, xCoord, yCoord, zCoord);
            final PacketFxTeleport targetMessage = new PacketFxTeleport(dimId, target.posX, target.posY, target.posZ);
            PacketUtils.sendToPlayersAround(sourceMessage, xCoord, yCoord + 1, zCoord, 64d, dimId);
            PacketUtils.sendToPlayersAround(targetMessage, target.posX, target.posY, target.posZ, 64d, dimId);
        }
    }
}
