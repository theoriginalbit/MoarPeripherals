...
public class TileFireworks extends TileInventory implements IActivateAwareTile, IBreakAwareTile {
    ...
    private static final ItemStack GLOWSTONE = new ItemStack(Item.glowstone);
    private static final ItemStack GUNPOWDER = new ItemStack(Item.gunpowder);
    private static final ItemStack DIAMOND = new ItemStack(Item.diamond);
    private static final ItemStack PAPER = new ItemStack(Item.paper);
    ...
    @LuaFunction
    public ArrayList<String> inspectFireworkRocket(int id) throws Exception {
        if (!bufferRocket.containsItemStackWithId(id)) {
            throw new Exception("No Firework Rocket with that ID found");
        }
        return getDescription(bufferRocket, id);
    }

    @LuaFunction
    public ArrayList<String> inspectFireworkStar(int id) throws Exception {
        if (!bufferStar.containsItemStackWithId(id)) {
            throw new Exception("No Firework Star with that ID found");
        }
    ...
    private static ItemStack colorToDye(int color) {
        return new ItemStack(Item.dyePowder, 1, 15 - color);
    }

    private static ArrayList<String> getDescription(QueueBuffer buffer, int id) {
    ...
    private static void addFireworkRocketInfo(NBTTagCompound tag, ArrayList<String> list) {
        ...
        NBTTagList nbttaglist = tag.getTagList("Explosions");

        if (nbttaglist != null && nbttaglist.tagCount() > 0) {
            for (int i = 0; i < nbttaglist.tagCount(); ++i) {
                final NBTTagCompound compound = (NBTTagCompound) nbttaglist.tagAt(i);
            ...
            for (j = 0; j < i; ++j) {
                ...
                for (int l = 0; l < 16; ++l) {
                    if (k == ItemDye.dyeColors[l]) {
                        flag1 = true;
                        s = s + StatCollector.translateToLocal("item.fireworksCharge." + ItemDye.dyeColors[l]);
                        break;
                    }
                }
            ...
            for (k = 0; k < j; ++k) {
                ...
                for (int i1 = 0; i1 < 16; ++i1) {
                    if (j1 == ItemDye.dyeColors[i1]) {
                        flag4 = true;
                        s1 = s1 + StatCollector.translateToLocal("item.fireworksCharge." + ItemDye.dyeColors[i1]);
                        break;
                    }
                }
    ...
    private enum Head {
        ...
        private Head() {
            itemStack = new ItemStack(Item.skull, 1, ordinal());
        }
