...
public final class TurtleUtil {
    private static final Class<?> CLAZZ_I_TURTLE_TILE = ReflectionUtil.getClass("dan200.computercraft.shared.turtle" +
            ".blocks.ITurtleTile");
    private static final Method METHOD_GETACCESS = ReflectionUtil.getMethod(CLAZZ_I_TURTLE_TILE, "getAccess");
    ...
    public static ITurtleAccess getITurtle(World world, int x, int y, int z) {
        ...
        if (tile != null) {
            return (ITurtleAccess) ReflectionUtil.callMethod(tile, METHOD_GETACCESS);
        }
        return null;
    }
