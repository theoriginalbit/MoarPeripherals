...
public class WrapperMethod {
    ...
    public Object[] invoke(IComputerAccess access, ILuaContext context, Object[] arguments) throws Exception {
        ...
        if (arguments.length != luaParamsCount) {
            throw new Exception(String.format("expected %d arg(s), got %d", luaParamsCount, arguments.length));
        }
        ...
        for (int i = 0; i < args.length; ++i) {
            if (IComputerAccess.class.isAssignableFrom(javaParams[i])) {
                ...
                if (convert == null) {
                    throw new Exception(String.format("expected %s, got %s", LuaType.getLuaName(javaParams[i]), LuaType.getLuaName(arguments[i].getClass())));
                }
                args[i] = convert;
            } else {
